 ...
 * is called, they will either finish their current job by default or keep taking jobs
 * from the queue until it is empty and then they will get destroyed.
 *
 * The flag can either be thpool_immediate (-1) or thpool_graceful (0).
 *
 * @param threadpool a pointer to the threadpool structure you want to destroy
 * @param flag a flag for shutdown
 */
void thpool_destroy(thpool_t* tp_p, thpool_destroy_flags_t flag);
